...
new class extends Component {
    ...
    public function updateOrder($items)
    {
        ...
        foreach ($items as $newIndex => $oldIndex) {
            ...
            $item['day_number'] = $newIndex + 1; // Re-index days correctly
            $newItinerary[] = $item;
        }
...
<div x-data="{ activeDay: null }">
    ...
    @push('styles')
        ...
        <style>
            ...
            .itinerary-header { background: #f8f9fa; transition: 0.2s; border-bottom: 1px solid #eee; }
            ...
            /* Dragging Style */
            .sortable-ghost { opacity: 0.4; background-color: #ffdce5 !important; }
        </style>
    @endpush
            ...
            <form wire:submit="save">
                <div class="row">
                    ...
                    <div class="col-md-7">
                            ...
                            <div id="itinerary-list" class="itinerary-scroll-container mb-3" 
                                 x-init="
                                    new Sortable($el, {
                                        ...
                                        onEnd: (evt) => {
                                            let items = Array.from(evt.to.children).map(el => el.getAttribute('data-index'));
                                ...
                                @foreach($itinerary as $index => $day)
                                    <div class="card mb-2 border-pink shadow-sm" wire:key="itinerary-day-{{ $index }}" data-index="{{ $index }}">
                                        <div class="card-header itinerary-header p-0 d-flex align-items-center justify-content-between">
                                            
                                            {{-- Handle + Title (Left Side) --}}
                                            <div class="d-flex align-items-center flex-grow-1 py-3">
                                                <div class="drag-handle">
                                                    <i class="fas fa-grip-vertical"></i>
                                                </div>
                                                <div class="flex-grow-1" style="cursor: pointer;" @click="activeDay = (activeDay === {{ $index }} ? null : {{ $index }})">
                                                    <span class="badge badge-pink mr-3">DAY {{ $day['day_number'] }}</span>
                                                    <span class="small font-weight-bold text-dark text-uppercase">
                                                        {{ $day['title'] ?: 'Untiled Day' }}
                                                    </span>
                                                </div>
                                            </div>

                                            {{-- Icon (Right Side) --}}
                                            <div class="pr-3" style="cursor: pointer;" @click="activeDay = (activeDay === {{ $index }} ? null : {{ $index }})">
                                        ...
                                        <div class="card-body p-3 bg-white" x-show="activeDay === {{ $index }}" x-cloak>
                                            <div class="form-group mb-3">
                                                ...
                                                <input type="text" wire:model.blur="itinerary.{{ $index }}.title" class="form-control font-weight-bold border-0 bg-light">
                                            </div>
                                            <div class="form-group mb-3">
                        ...
                        {{-- Trix Editor --}}
                        <div class="card shadow-sm mb-4">
                            <div class="card-header bg-white"><h5 class="mb-0 font-weight-bold">Overview</h5></div>
                            <div class="card-body">
